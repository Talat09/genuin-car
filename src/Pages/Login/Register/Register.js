        ...
        <div className='container w-50  mx-auto shadow-lg bg-primary  py-2  rounded my-5'>
            ...
            <Form onSubmit={handleSubmit} className='text-light text-start   justify-content-center'>
                ...
                <Form.Group className="mb-3" controlId="cheakboxs">
                    <input onClick={() => setAgree(!agree)} className='text-light ' type="checkbox" name="terms" id="terms" />
                    {/* <label className={agree ? 'ps-2' : 'ps-2 text-muted'} htmlFor="terms">Accept genuin cars Terms and Condition </label> */}
                    <label className={`ps-2${agree ? '' : 'ps-2 text-muted'}`} htmlFor="terms">Accept genuin cars Terms and Condition </label>
                </Form.Group>

                <Button
                    disabled={!agree}
                    className='bg-light text-primary fw-semibold px-5' variant="primary" type="submit">
                    Register
                </Button>
            </Form>
            <p className='text-light'>Already Have an Account? <Link to='/login' onClick={navigateLogin} className='text-light text-decoration-none   fw-semibold'>Login</Link> </p>
